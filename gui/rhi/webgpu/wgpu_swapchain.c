...
int bhs_gpu_swapchain_create(bhs_gpu_device_t device,
  ...
  WGPUSurfaceDescriptorFromCanvasHTML5Selector canvasDesc = {
    .chain = { .sType = WGPUSType_SurfaceDescriptorFromCanvasHTML5Selector },
  ...
  if (config->format == BHS_FORMAT_RGBA8_UNORM) sc->format = WGPUTextureFormat_RGBA8Unorm;
  ...
  WGPUSurfaceConfiguration surfConfig = {
    ...
    .width = sc->width,
    .height = sc->height,
    .presentMode = config->vsync ? WGPUPresentMode_Fifo : WGPUPresentMode_Mailbox,
...
int bhs_gpu_swapchain_next_texture(bhs_gpu_swapchain_t swapchain,
  ...
  sc->current_texture_wrapper.default_view = wgpuTextureCreateView(surfaceTex.texture, &viewDesc);
