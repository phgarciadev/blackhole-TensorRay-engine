...
static int compute_christoffel_kerr(const struct bhs_kerr *bh,
  ...
  return bhs_christoffel_compute(bhs_kerr_metric_func, pos, (void *)bh, h, out);
}

/**
 * Deriva da posição (simplesmente a velocidade)
 */
static struct bhs_vec4 dpos_dlambda(struct bhs_vec4 vel) { return vel; }
...
void bhs_geodesic_init_photon(struct bhs_geodesic *geo, struct bhs_vec4 pos,
  ...
  double spatial_norm = g.g[1][1] * kr * kr + g.g[2][2] * ktheta * ktheta +
                        g.g[3][3] * kphi * kphi +
...
int bhs_geodesic_step_rk4(struct bhs_geodesic *geo, const struct bhs_kerr *bh,
  ...
  /* k2 */
  struct bhs_vec4 pos2 = bhs_vec4_add(pos, bhs_vec4_scale(k1_pos, 0.5 * h));
  struct bhs_vec4 vel2 = bhs_vec4_add(vel, bhs_vec4_scale(k1_vel, 0.5 * h));
  struct bhs_vec4 k2_pos = dpos_dlambda(vel2);
  struct bhs_vec4 k2_vel = dvel_dlambda(bh, pos2, vel2);

  /* k3 */
  struct bhs_vec4 pos3 = bhs_vec4_add(pos, bhs_vec4_scale(k2_pos, 0.5 * h));
  struct bhs_vec4 vel3 = bhs_vec4_add(vel, bhs_vec4_scale(k2_vel, 0.5 * h));
...
int bhs_geodesic_step_adaptive(struct bhs_geodesic *geo,
  ...
  double error = sqrt(diff.t * diff.t + diff.x * diff.x + diff.y * diff.y +
                      diff.z * diff.z);
...
bhs_geodesic_propagate(struct bhs_geodesic *geo, const struct bhs_kerr *bh,
  ...
  int max_steps =
      config->max_steps > 0 ? config->max_steps : BHS_GEODESIC_MAX_STEPS;
  double escape_r = config->escape_radius > 0 ? config->escape_radius
  ...
  for (int i = 0; i < max_steps; i++) {
    ...
    if (config->disk_outer > 0 &&
        bhs_geodesic_is_in_disk(geo, config->disk_inner, config->disk_outer,
...
void bhs_geodesic_ray_from_camera(struct bhs_geodesic *geo,
  ...
  struct bhs_vec3 right = bhs_vec3_normalize(bhs_vec3_cross(forward, cam_up));
  struct bhs_vec3 up = bhs_vec3_cross(right, forward);

  /* Direção do raio para pixel (u, v) */
  double tan_fov = tan(fov * 0.5);
  struct bhs_vec3 ray_dir =
      bhs_vec3_add(forward, bhs_vec3_add(bhs_vec3_scale(right, u * tan_fov),
  ...
  /* ∂θ/∂(x,y,z) */
  double dtheta = (ray_dir.x * cos_theta * cos_phi +
                   ray_dir.y * cos_theta * sin_phi - ray_dir.z * sin_theta) /
                  r;

  /* ∂φ/∂(x,y,z) */
  double dphi =
      (-ray_dir.x * sin_phi + ray_dir.y * cos_phi) / (r * sin_theta + 1e-15);
