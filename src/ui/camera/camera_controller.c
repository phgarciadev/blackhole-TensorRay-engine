				  ...
				  double dt)
{
	...
	if (altitude < 1000.0f) altitude = 1000.0f; /* Min clamp */
	...
	if (cam->is_top_down_mode) {
		...
		move_speed = 2.0e9f * (float)dt; /* Original value for Normal mode */
	}
		...
		if (bhs_ui_mouse_down(ctx, 0)) { /* Left Button */
			...
			if (!is_dragging) {
				...
				if (!cam->is_top_down_mode) {
					...
					float pan_scale = altitude * 0.0015f; /* Pixel to World ratio */
					...
					/* dx>0 -> cam->x -= ... (Left) OK */
					cam->x -= (cos_y * dx + sin_y * dy) * pan_scale;
					...
					cam->z += (-sin_y * dx + cos_y * dy) * pan_scale;
				}
