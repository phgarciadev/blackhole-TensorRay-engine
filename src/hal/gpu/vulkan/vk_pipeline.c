...
#include "hal/gpu/vulkan/vk_internal.h"
#include <stdlib.h>
...
int bhs_gpu_shader_create(bhs_gpu_device_t device,
  ...
  VkResult result =
      vkCreateShaderModule(device->device, &create_info, NULL, &s->module);
...
int bhs_gpu_pipeline_create(bhs_gpu_device_t device,
  ...
  if (vkCreatePipelineLayout(device->device, &layout_info, NULL, &p->layout) !=
      VK_SUCCESS) {
    free(p);
  ...
  if (config->blend_state_count > 0) {
    ...
    color_blend_att.colorBlendOp = vk_blend_op_from_bhs(bs->color_op);
    ...
    color_blend_att.alphaBlendOp = vk_blend_op_from_bhs(bs->alpha_op);
  }
...
int bhs_gpu_pipeline_compute_create(
  ...
  if (vkCreatePipelineLayout(dev->device, &layout_info, NULL, &pipe->layout) !=
      VK_SUCCESS) {
    vkDestroyDescriptorSetLayout(dev->device, pipe->set_layout, NULL);
    free(pipe);
    return BHS_GPU_ERR_DEVICE;
  }
  ...
  if (vkCreateComputePipelines(dev->device, VK_NULL_HANDLE, 1, &pipeline_info,
                               NULL, &pipe->pipeline) != VK_SUCCESS) {
    vkDestroyPipelineLayout(dev->device, pipe->layout, NULL);
    vkDestroyDescriptorSetLayout(dev->device, pipe->set_layout, NULL);
    free(pipe);
    return BHS_GPU_ERR_DEVICE;
...
void bhs_gpu_pipeline_destroy(bhs_gpu_pipeline_t pipeline) {
  ...
  if (pipeline->set_layout)
    vkDestroyDescriptorSetLayout(pipeline->device->device, pipeline->set_layout,
                                 NULL);
...
int bhs_gpu_cmd_buffer_create(bhs_gpu_device_t device,
  ...
  VkDescriptorPoolSize pool_sizes[] = {
      ...
      {VK_DESCRIPTOR_TYPE_STORAGE_IMAGE, 1024}, /* Added for Compute */
  };
  ...
  vkCreateDescriptorPool(device->device, &pool_info, NULL, &c->descriptor_pool);
...
void bhs_gpu_cmd_buffer_destroy(bhs_gpu_cmd_buffer_t cmd) {
  ...
  if (cmd->descriptor_pool)
    vkDestroyDescriptorPool(cmd->device->device, cmd->descriptor_pool, NULL);
...
void bhs_gpu_cmd_begin(bhs_gpu_cmd_buffer_t cmd) {
  ...
  if (cmd->descriptor_pool)
    vkResetDescriptorPool(cmd->device->device, cmd->descriptor_pool, 0);
...
void bhs_gpu_cmd_set_index_buffer(bhs_gpu_cmd_buffer_t cmd,
  ...
  vkCmdBindIndexBuffer(cmd->cmd, buffer->buffer, offset,
                       is_32bit ? VK_INDEX_TYPE_UINT32 : VK_INDEX_TYPE_UINT16);
}
...
void bhs_gpu_cmd_bind_texture(bhs_gpu_cmd_buffer_t cmd, uint32_t set,
  ...
  if (vkAllocateDescriptorSets(dev->device, &alloc_info, &desc_set) !=
      VK_SUCCESS) {
    BHS_VK_LOG("aviso: falha ao alocar descriptor set (pool cheio?)");
    return;
  }
  ...
  vkCmdBindDescriptorSets(cmd->cmd, VK_PIPELINE_BIND_POINT_GRAPHICS,
                          cmd->current_pipeline_layout, set, 1, &desc_set, 0,
                          NULL);
}
...
void bhs_gpu_cmd_push_constants(bhs_gpu_cmd_buffer_t cmd, uint32_t offset,
  ...
  /* Usamos a união de todos os estágios suportados pelo framework para evitar
  ...
  */
  ...
  if (sc) {
    ...
    for (uint32_t i = 0; i < sc->image_count; i++) {
      if (&sc->texture_wrappers[i] == pass->color_attachments[0].texture) {
        is_swapchain = true;
  ...
  VkRenderPassBeginInfo rp_info = {
      ...
      .clearValueCount = 1,
      .pClearValues = &(VkClearValue){{{0.1f, 0.1f, 0.1f, 1.0f}}},
  };
...
void bhs_gpu_cmd_bind_compute_storage_texture(bhs_gpu_cmd_buffer_t cmd,
  ...
  if (!cmd->descriptor_pool)
    return;
  if (!pipeline->set_layout) {
    BHS_VK_LOG("erro: pipeline nao tem set_layout (nao eh compute?)");
    return;
  }
